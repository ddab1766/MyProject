...
class JobAppModelSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if instance.applied_status == ComCode.objects.get(code_id='BW0801000'):
            raise serializers.ValidationError('최종합격자는 수정할 수 없습니다.')
        ...
        if instance.applied_status != applied_status:
            if applied_status == ComCode.objects.get(code_id='BW0801000'):  # 최종합격
                ...
                try:
                    ...
                    raise serializers.ValidationError('사원정보 생성 오류!')
                ...
                try:
                    ...
                    raise serializers.ValidationError('계약정보 생성 오류!')
                ...
                try:
                    ...
                    raise serializers.ValidationError('합격 채용의뢰서 생성 오류!')
...
class SugubModelSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        Slack 알람
        '''
        text = '*[신규채용의뢰서]* [{}] - _{}_ 가 등록되었습니다.'.format(sugub.sugub_title, sugub.companyprofile.custname)
        slack_notify(text)
...
class CompanyProfileModelSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        Slack 알람
        '''
        text = '*[신규기업]* [_{}_]이 등록되었습니다.'.format(companyprofile.custname)
        slack_notify(text)
