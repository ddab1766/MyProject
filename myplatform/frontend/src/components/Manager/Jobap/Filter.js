...
const SugubEditForm = props => {
    ...
    const [searchType, setSearchType] = useState('applied_username');
    useEffect(() => {
    ...
    const handleDateFilter = () => {
        setFilter({...filter,['applied_at_min']:moment(date1).format('YYYY-MM-DD'),
            ['applied_at_max']:moment(new Date(new Date().setDate(date2.getDate()+1))).format('YYYY-MM-DD'),page : 1})
        paginate(1)
