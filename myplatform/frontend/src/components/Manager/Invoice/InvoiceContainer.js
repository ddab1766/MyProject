...
import {Dropdown, FlexboxGrid, List, Modal, Popover, Whisper} from "rsuite";
import {birthFormat, dateFormat, fileViewerUrl, phoneFormat, thousands} from "../../../function/common";
    ...
    return (
        <>
            <FlexboxGrid justify="space-between">
                ...
                {/*<FlexboxGrid.Item colspan={2} style={{...styleCenter,flexDirection: 'column',
                        alignItems: 'flex-start',
                        overflow: 'hidden'}}><div tyle={slimText}>환불</div></FlexboxGrid.Item>*/}
                    ...
                    <List.Item key={item.id} index={index} style={{padding:0}}>
                        <FlexboxGrid justify="space-between">
                            ...
                            <FlexboxGrid.Item
                                ...
                                <div style={titleStyle}>
                                    <Link to={`Mng/sugub/${item.contract.sugub}`} target='_blank'>보기</Link>
                                </div>
                            </FlexboxGrid.Item>
                            <FlexboxGrid.Item
                                colspan={2}
...
{tabValue === 0 && <>
                            <FlexboxGrid.Item
                                ...
                                <div style={titleStyle}>{dateFormat(item.contract.contract_from)}</div>
                            </FlexboxGrid.Item>
                            <FlexboxGrid.Item
                                colspan={2}
                                style={{...styleCenter,flexDirection: 'column',alignItems: 'flex-start',overflow: 'hidden' }}>
                                {/*<div style={slimText}>계약종료</div>*/}
                                <div style={titleStyle}>{dateFormat(item.contract.contract_to)}</div>
                            </FlexboxGrid.Item>
                            <FlexboxGrid.Item
                                colspan={2}
                                style={{...styleCenter,flexDirection: 'column',alignItems: 'flex-start',overflow: 'hidden' }}>
                            ...
                            {/*<FlexboxGrid.Item
                                colspan={2}
                                style={{...styleCenter,flexDirection: 'column',alignItems: 'flex-start',overflow: 'hidden' }}>
                                <a href='javascript:void(0);' onClick={()=>{toggle();setData(item)}}>신청</a>
                            </FlexboxGrid.Item>*/}
            ...
            >
                    ...
                    <ModalBody className="p-1">
                        <RefundForm data={data} toggle={toggle} />
                    </ModalBody>
                </Modal>
        </>
    )
