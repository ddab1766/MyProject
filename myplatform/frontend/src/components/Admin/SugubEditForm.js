...
import {CardTitle, Col, Container, Form, FormGroup, Row, Spinner} from "reactstrap";
...
import {renderMuiCheckbox, renderNumberField} from "@/utils/renderUtils";
...
const number = value =>
    value && value<1 ? '1이상이어야 합니다.' : undefined
const SugubEditForm = (props) => {
    const {handleSubmit, error, submitting,sugubs} = props;
    const {sugub_career_gb} = props;
    const [isSelected, setIsSelected] = useState(false);
    const [comcode, setComcode] = useState(props.comcode);
    ...
    useEffect(() => {
        //초기값세팅 직종코드중,소
        setJikTopcdM(sugub.sugub_jikjong_top.code_id);
        setJikTopcdS(sugub.sugub_jikjong_mid.code_id);
    }, []);
    ...
    // 경력구분
    useEffect(()=>{
        if(sugub_career_gb === undefined || sugub_career_gb === null) return
        if(typeof sugub_career_gb =='object'){
            console.log('props.sugub_career_gb', sugub_career_gb)
            if(sugub_career_gb.code_id === 'AB0200000'){
                setCareer(true)
            } else {
                console.log('?1111')
                setCareer(false);
    ...
    return sugub ? (
        <>
            <Container>
                ...
                {/*<div className="title">
                    <h3 className="text-md-left">수급상태변경<br/>
                    <button className="btn btn-primary">기업정보</button>
                    </h3>
                </div>*/}
                ...
                <Form onSubmit={handleSubmit} className="sugubForm" >
                    ...
                    <Row>
                        <Col className="text-center">
                            <button className="btn btn-lg btn-outline-info " onClick={() => history.push('/Mng/admin/sugub')}>뒤로가기</button>
...
function mapStateToProps(state, props) {
    const selector = formValueSelector('SugubEditForm');
