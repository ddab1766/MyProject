...
import {renderInputField, renderSelectField, renderInputNumberField} from "@/utils/renderUtils_rsuite";
                ...
                <CardBody>
                    <fieldset className="form-group">
                        <Field name="sugub_career_gb" label="원하시는 경력이 있으신가요?" component={renderSelectField}
                               ...
                               // code_topcd={null}
                               // type="text"
                               options={comCode.filter(v=>v.code_topidx === 'AB' && v.code_topcd === null)}
                               placeholder={'선택해주세요'}
                               required={true}
                               onChange={(e, value)=>onChangeCareer(e, value)}
                        />
                    </fieldset>
                    {renderCareerInput && (
                        <><label>경력구간을 선택해주세요.(슬라이더를 조정해주세요)</label>
                            <div className="input-group">
                                <Row>
                                    <Col md="3" xs={4}>
                                        <fieldset className="form-group">
                                            <Field name="career_start" component={renderInputNumberField}
                                                   type="number"
                                                   value={careerRange[0]}
                                                   onChange={(e, value)=>{
                                                       setCareerRange([value, careerRange[1] ])
                                                   }}
                                                   required={true}
                                                // disabled={true}
                                                   inputAddon={true}
                                    ...
                                    <Col md="3" xs={4}>
                                        <fieldset className="form-group">
                                            <Field name="career_end" component={renderInputField}
                                                   type="number"
                                                   onChange={(e, value)=>{
                                                       setCareerRange([careerRange[0], value])
                                                   }}
                                                   value={careerRange[1]}
                                                   required={true}
                                                // disabled={true}
                    ...
                    <fieldset className="form-group">
                        <Field name="education_cd" label="원하시는 학력이 있으신가요?" component={renderSelectField}
                               options={comCode.filter(v=>v.code_topidx === 'AO' && v.code_topcd === null)}
                               required={true}
                               placeholder={'선택해주세요'}
                        />
                    </fieldset>
                    <fieldset className="form-group">
                        <Field name="hire_count" label="채용인원은 몇명인가요?" component={renderInputNumberField}
                               // postfix={"명"}
                               required={true}
                        />
                    </fieldset>
