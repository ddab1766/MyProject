...
import { Modal, Tag, TagGroup } from "rsuite";
          ...
          <Row>
            <Col md={2} className="text-center">
              <div className="thumbnail img-circle">
                <Avatar src={partner.company_logo} style={{ width: "90px", height: "90px" }} />
              </div>
            </Col>
            <Col md={6}>
              {authenticated ? (
                ...
                >
                  <span style={{ fontSize: "1.3em" }}>{partner.custname}</span>
                </Link>
              ) : (
                <Link to={"#"}>
              ...
              {/*서비스 가능지역 */}
              <TagGroup>
                {partner.service_address.length > 0 &&
                  partner.service_address.map((data) => {
                    return (
            ...
            <Col md={4}>
              <div className="partners-stats">
                <ul>
                  ...
                  <li>
                    <img src={Applicant} width={20} height={20} />
                    <label>진행건수</label>
                    <span style={{ textAlign: "right", float: "right" }}>{partner.jobad_count} 건</span>
                  </li>
                  <li>
                    <img src={Review} width={20} height={20} />
                    <label>리뷰</label>
                    <span style={{ textAlign: "right", float: "right" }}>
                      {partner.hr_sugub_reviews && partner.hr_sugub_reviews.length} 건
                      {partner.hr_sugub_reviews && partner.hr_sugub_reviews.length > 0 && (
                        <>
                          (평점:
                          {partner.hr_sugub_reviews
                            .map((v) => {
                              return v.point_avg;
                            })
