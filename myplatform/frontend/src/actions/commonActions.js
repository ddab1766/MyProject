...
export function getComCode() {
  return axios.get(apiUrls.COMMON).then(({ data }) => {
...
export function getHrCard() {
  return axios.get(vUrls.HR_PROFILE_CARD).then(({ data }) => {
...
export function getHrProfile() {
  ...
  return function (dispatch) {
    defaultClient.get(vUrls.HR_PROFILE + "simple/").then(({ data }) => {
      dispatch({
        type: CommonTypes.GET_API_DATA,
        payload: data,
      });
...
export function getHrList(params) {
  ...
  if (params) {
    return defaultClient
      .get(vUrls.HR_PROFILE + `${params.id}/`, {
        params: {
          // is_expose: true
        },
      })
