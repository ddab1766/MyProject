...
from .models import User, ComIdx, ComCode, User, Jikjong, CompanyBaseInfo, Notification, Qna, FAQ, \
    Point, Alarm, AlarmSetting, Demand
from user_profile.models import UserProfile, UserSpecial, Resume
from company_profile.models import CompanyProfileCoWorker, SugubSuccess
from hr_profile.models import HrProfileCoWorker, HrProfile
...
class RecommendSugubSerializer(serializers.ModelSerializer):
    class Meta:
        model = SugubSuccess
        fields = '__all__'


class DemandModelSerializer(serializers.ModelSerializer):
    class Meta:
        model = Demand
        fields = '__all__'

    hrprofile = serializers.PrimaryKeyRelatedField(source="hr_profile.hrprofile", allow_null=True, required=False,
